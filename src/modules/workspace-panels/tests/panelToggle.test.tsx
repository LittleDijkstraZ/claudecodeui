...
function Harness() {
  ...
  return <><button onClick={() => actions?.togglePanel('shell')}>Shell</button><button onClick={() => actions?.togglePanel('files')}>Files</button><button onClick={() => actions?.openPanel('shell')}>Open shell explicitly</button>
    <output>{`${panel?.open}:${panel?.tab}:${[...(panel?.visited ?? [])].join(',')}`}</output><textarea aria-label="retained terminal" value={draft} onChange={event => setDraft(event.target.value)} /></>;
}
test('repeat tab selection collapses while explicit navigation remains idempotent and retains content', () => {
  ...
  fireEvent.click(screen.getByText('Shell')); expect(screen.getByRole('status').textContent).toBe('false:shell:shell');
