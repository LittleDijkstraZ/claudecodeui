...
export function TerminalConflictNotice({ records, sessionId }: { records?: NormalizedMessage[]; sessionId: string | null }) {
  ...
  // A later delivered human turn supersedes the old refusal, but its failed bubble stays reviewable.
  if (after.some(record => record.role === 'user' && (!record.delivery || record.delivery === 'delivered'))) return null;
