...
function RecordedText({ content, label, patch = false }: { content: string; label: string; patch?: boolean }) {
  ...
  // Remember character offsets so text pages stay bounded without losing earlier positions.
  const [starts, setStarts] = useState([0]);
  // Show one bounded page of a large recorded edit at a time.
...
/** Used by chat's changes review to render recorded edits without inventing unknown pre-write content. */
