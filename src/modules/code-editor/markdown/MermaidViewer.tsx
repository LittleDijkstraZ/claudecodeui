...
import {
  ...
  pinchDiagram,
  zoomDiagramAt,
} from '@/modules/code-editor/markdown/mermaidViewport';
import type { DiagramPoint, DiagramSize, DiagramTransform } from '@/shared/types';
...
/** Used by code-editor's MermaidDiagram to inspect its existing SVG locally without rendering it again. */
export default function MermaidViewer({ svg, onClose }: MermaidViewerProps) {
  ...
  // Update the displayed SVG geometry as the user zooms, fits or pans it.
  const [transform, setTransform] = useState<DiagramTransform>({ x: 0, y: 0, scale: 1 });
  // Display a grabbing cursor while a captured pointer is manipulating the diagram.
  const [isDragging, setIsDragging] = useState(false);
  // Retain viewport maximization when native fullscreen is unavailable.
