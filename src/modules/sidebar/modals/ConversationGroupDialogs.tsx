...
import type { CreatedGroupConversation, LLMProvider, Project } from '@/shared/types';
import { createGroupConversation } from '@/shared/api';
...
function AssignConversationDialog({ sessionId }: { sessionId: string }) {
  ...
  // Choose whether assignment targets an existing group or creates one.
  const [isNewGroup, setIsNewGroup] = useState(false);
  // Retain the new group name until creation succeeds.
  const [name, setName] = useState('');
  // Prevent repeated mutations and keep the dialog open while saving.
  const [isSaving, setIsSaving] = useState(false);
  // Display the last mutation failure without losing the selected values.
...
function NewGroupConversationDialog({ groupId, projects, selectedProject, onCreated }: ConversationGroupDialogsProps & { groupId: string }) {
  ...
  // Retain the chosen remote project while creating the grouped conversation.
  const [projectId, setProjectId] = useState(selectedProject?.projectId ?? projects[0]?.projectId ?? '');
  // Keep the selected remote provider for the new conversation.
...
/** Used by the project-workspace module to assign sessions and create grouped conversations. */
