type UnreadRecord = Record<string, { eventId: string; unread: boolean }>;
...
export function recordHubUnread(remoteId: string, sessionId: string, eventId: string) {
  ...
  if (records[sessionId]?.eventId !== eventId) { records[sessionId] = { eventId, unread: true }; save(remoteId, records); }
...
export function readHubConversation(remoteId: string, sessionId: string) {
  const records = read(remoteId);
  if (records[sessionId]) { records[sessionId].unread = false; save(remoteId, records); }
}
