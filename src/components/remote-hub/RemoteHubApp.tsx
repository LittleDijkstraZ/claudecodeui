...
import { useConversationGroupDrag } from '../sidebar/hooks/useConversationGroupDrag';

import { changeHubGroups, loadHubGroups, memberKey, moveHubMember, normalizeConversation, remoteRequest, type HubConversation, type HubGroup, type HubGroupState, type HubRemote } from './hubClient';
import { useHubConnections } from './useHubConnections';
...
function Hub() {
  ...
  useEffect(() => {
    void fetch('/hub-api/config').then(r => r.json()).then(data => setRemotes(data.remotes)).catch(() => setError('无法读取连接配置'));
    const reload = () => {
  ...
  useEffect(() => {
    for (const remote of remotes) {
      ...
      void (async () => {
        const user = await remoteRequest(remote.id, '/api/auth/user');
        ...
        const snapshot = await remoteRequest(remote.id, '/api/conversation-groups');
        ...
        for (const group of snapshot.groups ?? []) {
          ...
          let offset = 0,
            hasMore = true;
          while (hasMore) {
            const page = await remoteRequest(remote.id, `/api/conversation-groups/${encodeURIComponent(group.id)}/sessions?limit=100&offset=${offset}`);
  ...
  const loadProject = async (remoteId: string, projectId: string, more = false, preserveDepth = false) => {
    ...
    try {
      ...
      const data = await remoteRequest(remoteId, `/api/projects/${encodeURIComponent(projectId)}/sessions?limit=100&offset=${offset}`);
      ...
      while (loaded.length < needed && loaded.length < (project?.sessionMeta?.total ?? 0)) {
        const next = await remoteRequest(remoteId, `/api/projects/${encodeURIComponent(projectId)}/sessions?limit=100&offset=${loaded.length}`);
    ...
    {sidebarOpen && <aside className="absolute inset-y-0 left-0 z-30 flex w-[320px] max-w-[88vw] flex-col border-r border-border bg-card md:relative md:max-w-none" data-testid="hub-sidebar">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
        ...
        {(mode === 'recent' || mode === 'running') && <>{allConversations.filter(c => mode !== 'running' || states[c.remoteId]?.running.includes(c.sessionId)).filter(filtered).sort((a, b) => Date.parse(b.lastActivity ?? '') - Date.parse(a.lastActivity ?? '')).map(c => renderRow(c))}{mode === 'recent' && remotes.filter(r => (states[r.id]?.total ?? 0) > (states[r.id]?.conversations.length ?? 0)).map(r => <button key={r.id} className="p-2 text-xs text-primary" onClick={() => {
            void remoteRequest(r.id, `/api/providers/sessions/recent?limit=100&offset=${states[r.id].conversations.length}`).then(data => setStates(current => ({
              ...current,
              [r.id]: {
                ...current[r.id],
...
function HubDialog({
  ...
  groups: HubGroup[];
  remotes: HubRemote[];
  states: Record<string, import('./hubClient').HubRemoteState>;
  close: () => void;
  ...
  const submit = async () => {
    ...
    try {
      if (modal.kind === 'group') {
        ...
        let member = created;
        if (!member) {
          const result = await remoteRequest(remoteId, '/api/providers/sessions', {
            method: 'POST',
            body: JSON.stringify({
              provider,
              projectPath: project.fullPath
