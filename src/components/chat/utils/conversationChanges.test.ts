...
import { deriveConversationChanges } from './conversationChanges';
...
test('subagent successful edits use the parent anchor and child tool identity even if the parent fails', () => {
  const parent = tool('Task', JSON.stringify({ description: 'Implement the parser' }), {
    ...
    subagentState: {
      isComplete: false, currentToolIndex: 2, childTools: [
        { toolId: 'child-1', toolName: 'Edit', toolInput: editInput, timestamp: new Date(timestamp), toolResult: { content: 'Updated', isError: false } },
        { toolId: 'child-2', toolName: 'Write', toolInput: { file_path: '/project/b', content: 'x' }, timestamp: new Date(timestamp), toolResult: null },
        { toolId: 'child-3', toolName: 'Edit', toolInput: editInput, timestamp: new Date(timestamp), toolResult: { content: 'Denied', isError: true } },
      ],
    },
  });
