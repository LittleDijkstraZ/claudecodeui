import type { ConversationChangeTurn, ConversationFileChange } from '../types/conversationChanges';
...
/** Derive recorded changes for the summary panel without reading files or interpreting shell commands. */
  ...
  messages.forEach((message, messageIndex) => {
    ...
    if (message.isToolUse && message.toolName) {
      ...
      addTool({
        ...
        status: message.status, streaming: Boolean(message.isStreaming), sourceMessageKey,
        sourceToolId: toolId, timestamp: message.timestamp,
      });
    }
    if (message.subagentState?.childTools) {
      const parentInput = asRecord(message.toolInput);
      const contextLabel = firstString(parentInput.description, parentInput.subagent_type) || message.toolName;
      message.subagentState.childTools.forEach((child, childIndex) => addTool({
        ...
        timestamp: child.timestamp, contextLabel,
      }));
    }
  });
