...
/** Read-only identity evidence from this remote's existing session transcript. */
export default function ModelIdentitySummary({ provider, sessionId, selectedModel, revision }: {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void authenticatedFetch(`/api/providers/claude/sessions/${encodeURIComponent(sessionId)}/active-model`, { signal: controller.signal })
        .then(async (response) => {
