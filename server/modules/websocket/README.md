...
5. Claude advertises `canInterruptQueuedMessages` and `canStopTask` on runtime state and reconnect snapshots. `chat.interrupt { sessionId, clientMessageId, requestId? }` interrupts the current reply while preserving queued messages, background tasks, and native stdin. It never resubmits the identified message. `chat.stop-task { sessionId, taskId, requestId? }` targets one running native task. Both reply to the requesting socket with a `status` frame (`text: "queued_input_interrupt"` or `"task_stop"`, `status: "completed" | "failed"`, optional `error`, and the supplied identifiers). These are action acknowledgements; queued delivery changes and task completion still require native events. Action failures must not mark a submitted message undelivered.
