...
export async function askClaudeSideQuestion(query: Query, question: string, signal: AbortSignal): Promise<string> {
  // SDK 0.3.263 ships this method in sdk.mjs but omits it from the public declarations.
  const native = query as Query & {
    askSideQuestion?: (question: string, options: { signal: AbortSignal }) => Promise<{ response: string; synthetic: boolean } | null>;
  };
  ...
  const result = await native.askSideQuestion(question, { signal });
